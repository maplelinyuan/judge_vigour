...
# 需要修改的url链接
give_me_url = 'http://www.okooo.com/soccer/league/615/schedule/12024/1-4417/?show=all'
    ...
    for group_index in range(0, group_total):
        ...
        for match_day_index in range(0, match_day_total):
            ...
            for single_match in driver.find_elements_by_xpath('//table[@id="team_fight_table"]/tbody/tr[@align="center"]'):
                ...
                # 进入详情页面
                this_match_handicap_find = WebDriverWait(driver, 30).until(lambda driver: driver.find_elements_by_xpath('//tr[@class="topjfbg jsThisMatch"]'))
                if len(this_match_handicap_find) == 0 or len(this_match_handicap_find[0].find_elements_by_xpath('td')) < 11:
                    driver.close()
                    windows = driver.window_handles
                    driver.switch_to.window(windows[0])
                    continue
                this_match_handicap = this_match_handicap_find[0]
