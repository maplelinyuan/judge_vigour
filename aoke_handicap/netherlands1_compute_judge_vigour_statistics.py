...
for item in coll.find({'league_name': '荷甲'}):
# for item in coll.find():
    ...
    if abs(vigour_difference) >= 0.8:
        ...
        if (abs(this_match_handicap_num) < 1.25 and this_match_handicap_num >= 0) or (abs(this_match_handicap_num) < 0.75 and this_match_handicap_num <= 0):
