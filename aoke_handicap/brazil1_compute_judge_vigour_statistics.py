...
for item in coll.find({'league_name': '巴西甲'}):
# for item in coll.find():
    ...
    if abs(vigour_difference) >= 0.9:
        ...
        if (abs(this_match_handicap_num) < 1.5 and this_match_handicap_num >= 0) or (abs(this_match_handicap_num) < 0.5 and this_match_handicap_num <= 0):
