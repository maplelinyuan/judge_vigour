...
for item in coll.find({'league_name': '欧冠'}):
# for item in coll.find():
    ...
    if abs(vigour_difference) >= 0.8:
        ...
        if (abs(this_match_handicap_num) < 2 and this_match_handicap_num >= 0) or (abs(this_match_handicap_num) < 0.75 and this_match_handicap_num <= 0):
